...
/**
 * An annotator which removes all xml tags (as identified by the tokenizer) and
 * possibly selectively keeps the text between them. Can also add sentence
 * ending markers depending on the xml tag.
 *
 * @author John Bauer
 */
public class CleanXmlAnnotator2 implements Annotator {
     ...
     * news articles we parse go: <br> &lt;text&gt; <br> &lt;turn&gt; <br>
     * &lt;turn&gt; <br> &lt;turn&gt; <br> &lt;/text&gt; <br> ... eg, no closing
     * &lt;/turn&gt; tags.
     */
    ...
    private final Pattern stripContents;

    public CleanXmlAnnotator2() {
        ...
        for (CoreLabel token : tokens) {
            ...
            if (tag == null) {
                ...
                if (removedText.length() > 0) {
                    ...
                    if (added && newTokens.size() > 1) {
                        ...
                        if (after != null) {
                            previous.set(AfterAnnotation.class, after
                                    + removedText);
                        } else {
                            previous.set(AfterAnnotation.class, removedText
                                    .toString());
                        }
                    }
                ...
                if (dateTagMatcher != null
                        ...
                        && dateTagMatcher.matcher(currentTagSet
                        .get(currentTagSet.size() - 1)).matches()) {
            ...
            if (tag.isEndTag) {
                while (true) {
                    ...
                    if (stripContents.matcher(tag.name).matches()) {
                        --strippingDepth;
                    }
